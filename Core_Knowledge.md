...
**Root Cause:** If a remediation script only deletes `.tbacct` files that match existing `WorkplaceJoin` registry keys, some orphaned `.tbacct` files can be left behind permanently. Over time, severe TokenBroker corruption can occur, causing the LicenseAcquisition task to fail.
**Fix:** A Session Revoke only drops the token in the cloud. You must physically wipe the cache by deleting ALL `.tbacct` files in `AppData\Local\Packages\Microsoft.AAD.BrokerPlugin_cw5n1h2txyewy\AC\TokenBroker\Accounts\`. However, this should only be done preemptively if orphaned registry entries exist, or reactively if the LicenseAcquisition task explicitly throws Error `0x87E10C0A`.
**Threshold:** A healthy PRT cache natively accumulates many `.tbacct` files (e.g., 15-40+) for modern apps like Teams, Edge, and Office. Do not preemptively purge files based on a low count threshold, as this will trigger an infinite rebuild loop.
